...
import {
  generateSalesData, generateForecast, aggregateByDate,
  STORES, PRODUCTS,
} from "@/lib/mock-data";
...
export default function Forecast() {
  ...
  const filteredData = useMemo(() => {
    let d = rawData;
    if (storeFilter !== "all") d = d.filter(r => r.storeId === storeFilter);
    if (productFilter !== "all") d = d.filter(r => r.productId === productFilter);
    return d;
  }, [rawData, storeFilter, productFilter]);

  const dailyData = useMemo(() => aggregateByDate(filteredData), [filteredData]);
  const forecast = useMemo(() => generateForecast(filteredData, horizon), [filteredData, horizon]);
  ...
  const avgPredicted = Math.round(totalPredicted / forecast.length);
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Predict future sales using the best-performing XGBoost model
        </p>
      </div>
      ...
      <div className="glass-card rounded-lg p-5">
        ...
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData}>
              ...
              <ReferenceLine
                x={dailyData[dailyData.length - 1]?.date}
        ...
        <div className="flex gap-6 mt-3 text-xs text-muted-foreground">
          <span className="flex items-center gap-1.5">
            <span className="w-4 h-0.5 bg-[hsl(200,70%,50%)] inline-block" /> Actual
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-4 h-0.5 bg-primary inline-block" style={{ borderTop: "2px dashed" }} /> Predicted
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-4 h-2 bg-primary/10 inline-block rounded-sm" /> Confidence Interval
          </span>
        </div>
      </div>
