    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Compare trained models and view evaluation metrics
        </p>
      </div>
      ...
      <div className="glass-card rounded-lg p-5 glow-border">
        ...
        <div className="flex flex-wrap gap-6">
          <div>
            ...
            <div className="text-lg font-semibold mt-1">{best.name}</div>
          </div>
          <div>
            <span className="stat-label">MAPE</span>
            <div className="stat-value mt-1">{best.mape}%</div>
          </div>
          <div>
            <span className="stat-label">R² Score</span>
            <div className="stat-value mt-1">{best.r2}</div>
          </div>
          <div>
            <span className="stat-label">MAE</span>
            <div className="stat-value mt-1">{best.mae}</div>
          </div>
          <div>
            <span className="stat-label">Training Time</span>
            <div className="stat-value mt-1">{best.trainingTime}s</div>
          </div>
        </div>
      </div>
        ...
        {metrics.map((m) => (
          <div
            ...
            className={`glass-card rounded-lg p-4 transition-all ${
              m.selected ? "ring-2 ring-primary/50" : ""
            }`}
          >
            ...
            <div className="space-y-2 text-sm">
              <MetricRow label="MAE" value={m.mae.toFixed(1)} />
              <MetricRow label="RMSE" value={m.rmse.toFixed(1)} />
              <MetricRow label="MAPE" value={`${m.mape}%`} />
              <MetricRow label="R²" value={m.r2.toFixed(2)} />
